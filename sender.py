...
class SenderUi(QMainWindow):
    ...
    def login(self):
        try:
            ...
            create_message_box('Informații de conectare incorecte!')
        except:
            create_message_box('Eroare generala')
    ...
    def send(self):
        try:
            ...
            self.message['Subiect'] = self.input_subject.text()
            self.message['De la'] = self.input_from.text()
            self.message['Catre'] = self.input_to.text()
            self.message.attach(plain_text)
            self.server.sendmail(self.input_from.text(), [self.input_to.text()], self.message.as_string())

            create_message_box('E-mail trimis cu succes')
            self.reset()

        except:
            create_message_box('Eroare la trimiterea e-mailului')
